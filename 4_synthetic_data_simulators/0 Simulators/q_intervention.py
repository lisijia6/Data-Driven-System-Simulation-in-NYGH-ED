...
if __name__ == "__main__":
    ...
    q_3 = multi_class_single_station_fcfs(lambda_=0.9, classes=[0, 1], probs=[0.1, 0.9],
                                         mus=[1, 0.95], prob_speedup=[0.0, 0.0], mus_speedup=[2, 0.95],
                                         servers=1) # laplace_params=[0, 0.5]
    q_3.simulate_q(customers=40000, runs=1, system_type=1)  # 20000
    start = time.time()
    q_3.generate_data()
    end = time.time()
    time_to_run = round((end - start), 3)
